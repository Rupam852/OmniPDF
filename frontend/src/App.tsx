...
export default function App() {
  ...
  const handleProcessTool = async (files: File[], options: Record<string, any>) => {
    ...
    if (!user) {
      setAuthError('Please Login first to utilize PDF tools and store execution logs.');
    ...
    try {
      ...
      console.log(`Processing tool ${selectedTool.id} for user ${user.uid}`);
      ...
      if (selectedTool.id === 'merge') {
        ...
        // Log this tool execution to backend PostgreSQL DB
        ...
        <div style={appStyles.authButtons}>
          {user ? (
            <div style={appStyles.userInfo}>
              ...
              <button onClick={handleSignOut} className="btn-primary" style={{ backgroundColor: '#dc2626' }}>
                Sign Out
              </button>
            </div>
          ) : (
            <>
              <button 
                onClick={() => { setAuthMode('login'); setAuthError(null); setIsAuthModalOpen(true); }} 
                style={appStyles.loginBtn}
              >
                Login
              </button>
              <button 
                onClick={() => { setAuthMode('signup'); setAuthError(null); setIsAuthModalOpen(true); }} 
